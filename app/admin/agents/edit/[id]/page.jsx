...
// import axios from "axios"; // Using fetch API for consistency now
import { ArrowLeft, Upload, X, Loader2 } from "lucide-react"; // Eye/EyeOff removed as password not in agent model
import {
  getAdminToken,
  isAdminTokenValid,
...
export default function EditAgentPage() {
  ...
  const [fetchLoading, setFetchLoading] = useState(true); // For initial agent data fetch
  // const [showPassword, setShowPassword] = useState(false); // Password field removed, so not needed
  const [form, setForm] = useState({
    ...
    // password: "", // Password is not in the agent model, remove from form state
  });

  const [avatar, setAvatar] = useState(null); // For new file upload
  const [avatarPreview, setAvatarPreview] = useState(null); // For displaying preview of new/existing avatar
  const [existingCuid, setExistingCuid] = useState(null); // To keep track of the original cuid path if no new file is uploaded
  ...
  const fetchAgent = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/agents/admin/${agentId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (responseData.success && responseData.data) {
        ...
        setForm({
            ...
            name: agent.name || "", // Direct name if available
            first_name: agent.first_name || (agent.name?.split(' ')[0] || ''), // Fallback to splitting name
            last_name: agent.last_name || (agent.name?.split(' ').slice(1).join(' ') || ''), // Fallback to splitting name
            nationality: agent.nationality || "",
            orn_number: agent.orn_number || "",
            orn: agent.orn || "",
            brn: agent.brn || "",
            mobile: agent.mobile || "",
            ...
            seo_keywork: agent.seo_keywork || "",
            seo_description: agent.seo_description || "",
            status: agent.status !== undefined ? agent.status : 1,
            // password: "", // Never populate password field
        });

        if (agent.cuid) { // Check for image path in cuid
          setExistingCuid(agent.cuid);
          setAvatarPreview(getImageUrl(agent.cuid));
        }

        console.log("✅ Agent data loaded:", agent);
      } else {
        throw new Error(responseData.message || "Failed to fetch agent data.");
      }
    } catch (error) {
      console.error("❌ Error fetching agent:", error);
      showError(error.message || "Failed to load agent data.");
      router.push("/admin/agents"); // Redirect on error
    } finally {
  ...
  useEffect(() => {
    if (isAuthenticated && agentId) { // Ensure agentId is available
      fetchAgent();
    }
  }, [isAuthenticated, agentId, fetchAgent]);
  ...
  const handleAvatarUpload = (e) => {
    ...
    reader.onload = (ev) => setAvatarPreview(ev.target.result); // Display preview
    reader.readAsDataURL(file);
    showSuccess("Avatar selected");
  };

  const removeAvatar = () => {
    setAvatar(null); // Clear new file
    setAvatarPreview(null); // Clear preview
    setExistingCuid(null); // Indicate existing avatar should be removed
  };
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const submitToastId = showLoadingToast("Updating agent...");
    ...
    try {
      ...
      Object.entries(form).forEach(([key, value]) => {
        ...
        // Skip fields that are not meant for update or are empty
      ...
      // If a new avatar is selected, append it under the key 'image'
      if (avatar) {
        formData.append("image", avatar);
      } else if (existingCuid === null) {
        // If existingCuid was cleared, it means the user explicitly removed the avatar.
        formData.append("remove_avatar", "true");
      }

      // Determine the correct endpoint based on whether an image is being uploaded
      const endpoint = avatar ? `/api/agents/admin/${agentId}/with-image` : `/api/agents/admin/${agentId}`;
      ...
      if (responseData.success) {
        ...
        showSuccess(responseData.message || "Agent updated successfully");
        router.push("/admin/agents"); // Redirect to agents listing page
      } else {
        throw new Error(responseData.message || "Failed to update agent.");
      }
    } catch (error) {
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600 font-medium">
            {authLoading ? "Verifying authentication..." : "Loading agent data..."}
          </p>
        </div>
      </div>
    );
