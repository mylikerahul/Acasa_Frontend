...
export default function CommunityPage() {
  const [formData, setFormData] = useState(INITIAL_COMMUNITY_FORM);
  const [search, setSearch] = useState("");
  const [showAddDbModal, setShowAddDbModal] = useState(false);
  const [modalTab, setModalTab] = useState("community"); // "community" | "subCommunity"
  ...
  const handleDbSubmit = (e) => {
    e.preventDefault();
    console.log("Add New Community Database:", {
      city: dbCity,
      name: dbName,
    });
  ...
  const handleSubSubmit = (e) => {
    e.preventDefault();
    console.log("Add New Sub Community:", subForm);
    setShowAddDbModal(false);
    setSubForm({
      country: "",
      subName: "",
  ...
  const communityPlainTextDesc = formData.description
    .replace(/<[^>]*>/g, "")
    .split(/\s+/)
    .filter(Boolean);

  const subPlainTextDesc = subForm.description
    .replace(/<[^>]*>/g, "")
    .split(/\s+/)
    .filter(Boolean);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            {/* LEFT: Add / Edit Community main form */}
            <div className="lg:col-span-2 space-y-6">
              ...
              >
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    <ul className="mt-3 space-y-1 text-xs text-gray-700">
                      ...
                      <li>
                        Content is {communityPlainTextDesc.length || 1} words
                        long. Consider using at least 600 words.
                      </li>
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-gray-500">
                      {communityPlainTextDesc.length} words. Try to write at
                      least 600 words.
                    </p>
                  </div>
                </div>
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3">
                  <button
                    type="button"
                    onClick={() => setFormData(INITIAL_COMMUNITY_FORM)}
      ...
      {/* POPUP: Add Community Data (2 tabs: Community / Sub Community) */}
      {showAddDbModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white w-full max-w-xl rounded-xl shadow-xl border border-gray-200">
            ...
            {/* Tabs */}
            <div className="px-5 pt-3 border-b border-gray-200 flex gap-4 text-sm">
              <button
                type="button"
                onClick={() => setModalTab("community")}
              ...
              <button
                type="button"
                onClick={() => setModalTab("subCommunity")}
                className={`pb-2 border-b-2 ${
                  modalTab === "subCommunity"
                    ? "border-amber-600 text-amber-600 font-semibold"
                    : "border-transparent text-gray-500 hover:text-gray-800"
                }`}
              >
                ...
                <form onSubmit={handleSubSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSubForm((p) => ({ ...p, country: e.target.value }))
                      }
                      required
                      className="w-full h-11 px-4 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSubForm((p) => ({ ...p, subName: e.target.value }))
                      }
                      required
                      className="w-full h-11 px-4 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSubForm((p) => ({ ...p, seoTitle: e.target.value }))
                      }
                      maxLength={60}
                      className="w-full h-11 px-4 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      {subForm.seoTitle.length} / 60 – This is what will
                      appear in the first line when this shows up in the
                      search results.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      {subForm.seoDescription.length} / 160 – This is what
                      will appear as the description when this shows up in the
                      search results.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="mt-3 space-y-1 text-xs text-gray-700">
                      ...
                      <li> Use Focus Keyword in the content.</li>
                      <li>
                        Content is {subPlainTextDesc.length || 1} words long.
                        Consider using at least 600 words.
                      </li>
                    </ul>
                  </div>

                  {/* Optional: Sub community description with Quill */}
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                    ...
                    <div className="border border-gray-300 rounded-lg overflow-hidden bg-white">
                      <ReactQuill
                        ...
                        onChange={(val) =>
                          setSubForm((p) => ({ ...p, description: val }))
                        }
                        modules={quillModules}
                        formats={quillFormats}
                        placeholder="Describe sub community..."
                      />
                    </div>
                  </div>
