...
// Chart options – styled to look like the screenshot
    ...
    tooltip: {
      ...
      callbacks: {
        label: (context) => {
          ...
          if (label) label += ": ";
          label += `${context.parsed.y.toLocaleString()} projects`;
          return label;
        },
      },
    },
...
export default function InternationalProjectsChart() {
  const [chartData, setChartData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Fallback empty data (12 months, 0 values)
  ...
  // Fetch monthly analytics for current year (backend same)
  const fetchMonthlyAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/v1/projects/analytics/monthly`,
        {
          params: { year },
      ...
      if (response.data.success) {
        ...
        if (monthly && monthly.data) {
          const { data, months: monthLabels } = monthly;
          const labels = monthLabels || MONTHS;

          // We only use "international" as Development Projects
          const developmentData = labels.map((month) => {
            return data[month]?.international || 0;
          });
  ...
  // Loading UI (simple, matches card style)
  if (loading && !chartData) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm sm:text-base font-semibold text-gray-900">
            International Projects from the last 12 month
          </h3>
          {/* simple three-line menu icon */}
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-4 sm:p-5">
      {/* Header same style as screenshot */}
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm sm:text-base font-semibold text-gray-900">
          International Projects from the last 12 month
        </h3>
