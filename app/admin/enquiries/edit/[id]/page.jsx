...
export default function EditEnquiryPage() {
  ...
  useEffect(() => {
    const fetchEnquiry = async () => {
      if (!isAuthenticated || !enquiryId) return;

      setLoading(true);
      try {
        const result = await apiRequest(`/api/v1/enquiries/${enquiryId}`);
        ...
        if (result.success && result.data) {
          ...
          setFormData({
            ...
            lost_status: data.lost_status || "",
            exclusive_status: data.exclusive_status || "",
            construction_status: data.construction_status || "",
            agent_id: data.agent_id || "",
            message: data.message || "",
            agent_activity: data.agent_activity || "",
            admin_activity: data.admin_activity || "",
            drip_marketing: Boolean(data.drip_marketing),
            contact_date: formatDate(data.contact_date),
            property_image: null,
            resume: null,
          });
          ...
          if (data.property_image) {
            setExistingImage(data.property_image);
          }
          if (data.resume) {
        ...
        if (err.message === "Not found") {
          ...
          toast.error(err.message || "Error fetching enquiry");
        }
      } finally {
        setLoading(false);
      }
    ...
    fetchEnquiry();
  }, [isAuthenticated, enquiryId, apiRequest]);
  ...
  useEffect(() => {
    ...
    const changed = Object.keys(formData).some((key) => {
      ...
      if (key === "drip_marketing") {
        return formData[key] !== Boolean(originalData[key]);
      }
      if (key === "contact_date") {
        return formData[key] !== formatDate(originalData[key]);
      }
      return String(formData[key] || "") !== String(originalData[key] || "");
    });

    setHasChanges(changed || removeExistingImage || removeExistingResume);
  }, [formData, originalData, removeExistingImage, removeExistingResume]);
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      if (removeExistingImage) {
        submitData.append("remove_property_image", "true");
      }
      if (removeExistingResume) {
        submitData.append("remove_resume", "true");
      }

      const result = await apiRequest(`/api/v1/enquiries/update/${enquiryId}`, {
        method: "PUT",
        body: submitData,
      });
  ...
  }, [formData, validateForm, hasChanges, enquiryId, apiRequest, originalData, removeExistingImage, removeExistingResume]);
  ...
  }, [enquiryId, apiRequest, hasChanges]);
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-[#f6f6f6] flex items-center justify-center">
          <div className="text-center">
            <Loader2 className="w-8 h-8 text-gray-700 animate-spin mx-auto mb-3" />
            <div className="text-sm text-gray-600">Loading enquiry data...</div>
          </div>
        </div>
      </>
    );
        ...
        <div className="max-w-[1200px] mx-auto px-4 py-6">
          ...
          <div className="flex items-center justify-between mb-6">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-[20px] font-semibold text-gray-800">
                    Edit Enquiry #{enquiryId}
                  </h1>
                  {hasChanges && (
                    <span className="px-2 py-0.5 text-[11px] bg-yellow-100 text-yellow-700 rounded">
                      Unsaved Changes
                    </span>
                  )}
                </div>
                <p className="text-[13px] text-gray-500 mt-0.5">
                  Last updated: {formatDateTime(originalData?.updated_at)}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => fastNavigate(`/admin/enquiries/view/${enquiryId}`)}
                className={btnSecondary}
              >
              ...
              <button
                ...
                disabled={submitting || submitSuccess || !hasChanges}
                className={btnPrimary}
              >
                {submitting ? (
          ...
          {originalData && (
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <span className={`px-3 py-1 text-[11px] rounded-full ${
                STATUS_OPTIONS.find(s => s.value === originalData.status)?.color || "bg-gray-100 text-gray-700"
              }`}>
                {originalData.status || "New"}
              </span>
              {originalData.priority && (
                <span className={`px-3 py-1 text-[11px] rounded-full ${
                  PRIORITY_OPTIONS.find(p => p.value === originalData.priority)?.color || "bg-gray-100 text-gray-700"
                }`}>
                  {originalData.priority}
                </span>
              )}
              {originalData.quality && (
                <span className={`px-3 py-1 text-[11px] rounded-full ${
                  QUALITY_OPTIONS.find(q => q.value === originalData.quality)?.color || "bg-gray-100 text-gray-700"
                }`}>
                  {originalData.quality}
                </span>
              )}
              <span className="text-[11px] text-gray-500 flex items-center gap-1">
                ...
                Created: {formatDateTime(originalData.created_at)}
              </span>
            </div>
          )}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {STATUS_OPTIONS.map((opt) => (
                          <button
                            ...
                            className={`px-3 py-2 text-[12px] border rounded transition-colors ${
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      {imagePreview ? (
                        <div className="relative">
                          ...
                          <span className="absolute bottom-2 left-2 px-2 py-1 bg-black/50 text-white text-[10px] rounded">
                            New Image
                          </span>
                        </div>
                        ...
                        <div className="relative">
                          ...
                          <a
                            href={`${API_BASE_URL}${existingImage}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="absolute bottom-2 right-2 px-2 py-1 bg-black/50 text-white text-[10px] rounded flex items-center gap-1 hover:bg-black/70"
                          >
                    ...
                    {formData.type === "career" && (
                      <div>
                        ...
                        {resumeFileName && !removeExistingResume ? (
                          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div className="flex items-center gap-2">
                              <FileText className="w-5 h-5 text-blue-500" />
                              <div>
                                <span className="text-[12px] text-gray-700 block truncate max-w-[150px]">
                                  {resumeFileName}
                                </span>
                                {existingResume && (
                                  <a
                                    href={`${API_BASE_URL}${existingResume}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                  ...
                  <div className={sectionCls}>
                    ...
                    <div className="space-y-3 text-[12px]">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-gray-800">#{originalData.id}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">Created At</span>
                        <span className="font-medium text-gray-800">
                          {formatDateTime(originalData.created_at)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">Updated At</span>
                        <span className="font-medium text-gray-800">
                          {formatDateTime(originalData.updated_at)}
                        </span>
                      </div>
                    </div>
                  </div>
